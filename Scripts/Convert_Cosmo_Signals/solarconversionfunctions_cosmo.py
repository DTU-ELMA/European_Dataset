...
def newHayDavies(Ib, Id, Ig, latitude, longitude, utcTime, orientationFunction):
    ...
    I_extraterrestial = zeros(Id.shape);
    ...
    # Get the beam, diffuse and ground components of radiation.

    Ib_t = zeros(Ib.shape);
    Ib_t[mask] = Ib[mask] * R[mask];

    #Calculate anisotropy index:

    A = zeros(I.shape);
    A[mask] = Ib[mask] / I_extraterrestial[mask];

    # Horizontal brightening diffuse correction factor
    f = zeros(I.shape);
    f[mask] = sqrt(Ib[mask]/I[mask]);

    #And go on

    Id_t = zeros(Id.shape);
    Id_t[mask] = Id[mask] * ((1-A[mask])*(1+cos(slope[mask]))/2*(1+f[mask]*sin(slope[mask]/2)**3) + A[mask]*R[mask]);

    Ig_t = zeros(I.shape);
    Ig_t[mask] = Ig[mask]*(1-cos(slope[mask]))/2 


    return Id_t + Ib_t + Ig_t;
...
def SolarPVConversion(data,c): # c is for config, apparently
