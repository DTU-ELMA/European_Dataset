...
directories = directories[directories.index('2013100100'):directories.index('2014010100')]
...
for directory in directories:
    ...
    if not lastmonth == date.month:
        ...
        np.savez_compressed(outdirts + '/' + wnduvname + '-' + str(lastyear) + '{:02d}'.format(lastmonth) + '.npz', data=tswind, dates=tstimes)
        np.savez_compressed(outdirts + '/' + wnduv100name + '-' + str(lastyear) + '{:02d}'.format(lastmonth) + '.npz', data=tswind100, dates=tstimes)
        np.savez_compressed(outdirts + '/' + downsolarname + '-' + str(lastyear) + '{:02d}'.format(lastmonth) + '.npz', data=tsdsolar, dates=tstimes)
        np.savez_compressed(outdirts + '/' + upsolarname + '-' + str(lastyear) + '{:02d}'.format(lastmonth) + '.npz', data=tsusolar, dates=tstimes)
        np.savez_compressed(outdirts + '/' + tmp2mname + '-' + str(lastyear) + '{:02d}'.format(lastmonth) + '.npz', data=tstmp, dates=tstimes)
