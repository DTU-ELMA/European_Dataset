...
void windLoop( size_t N, size_t M, const double windspeed[N][M], \
			const _Bool onshoremap[N][M], double onshoreHubHeight, double offshoreHubHeight,
			size_t onLen, const double onV[onLen], const double onPow[onLen],\
			size_t offLen, const double offV[offLen], const double offPow[offLen],\
			double result[N][M])
{
